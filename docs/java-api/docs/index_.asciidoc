...
use it. The `jsonBuilder` is a highly optimized JSON generator that
...
twitter, under a type called tweet, with id value of 1:
...
Note that you can also index your documents as JSON String and then you
...
The index API allows one to set the threading model the operation will
