...
The update API allows you to update a document based on a script provided:
...
The update API also supports passing a partial document, which will be merged into the existing document (simple
...
If the document `index/type/1` already exists, after this operation we will have a document like:
